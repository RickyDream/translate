...
'''
Python wrapper for the Aria2 RPC server.
'''
...
# default keys
DEFAULT_KEYS = ['gid','status','dir','downloadSpeed','uploadSpeed','totalLength', 'completedLength']
...
class Aria2RpcServer(object):
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        print('{}: failed to kill PID {:d}'.format(
            self.__class__.__name__,
            self.process.pid
        ))
        self.kill()
...
#       protocol = ssl.PROTOCOL_TLSv1_2 # Python 3.4+
    ...
    def send_request(self, req_obj):
        ...
        try:
            # r = urllib.request.urlopen(self.uri, req)
            ...
            raise Aria2JsonRpcError(
                err_str,
                connection_error=(
                    '111' in err_str or isinstance(err, (urllib.error.URLError, ConnectionResetError))
                )
            )
...
# ----------------------------- Standard Methods ----------------------------- #
    ...
    def tellAll(self,numWaiting,numStopped, keys=None):
        keys = keys or ['totalLength', 'completedLength']
        total:list = self.tellActive(keys)
        activeTasks = copy.deepcopy(total)
        waiting = self.tellWaiting(0, numWaiting, keys)
        if waiting:
            total.extend(waiting)
        stopped = self.tellStopped(0, numStopped, keys)
        stoppedTasks = copy.deepcopy(stopped)
        if stopped:
            total.extend(stopped)
...
# --------------------------- Convenience Methods ---------------------------- #
    ...
    def get_global_status(self,gids=None, keys=None, type:str='all'):
        '''
        Print global status of the RPC server.
        '''
        globalStat = self.getGlobalStat()
        result = {}
        if globalStat:
            numWaiting = int(globalStat['numWaiting'])
            # numStopped = int(globalStat['numStopped'])
            numStopped = int(globalStat['numStoppedTotal'])
            keys = keys or DEFAULT_KEYS
            method = f"tell{type.title()}"
            if method == 'tellAll':
                totalTasks, activeTasks = self.tellAll(numWaiting, numStopped, keys)
            else:
                totalTasks,activeTasks = hasattr(self, method) and getattr(self, method)(keys)
            totalLength = sum(int(x['totalLength']) for x in totalTasks)
            completedLength = sum(int(x['completedLength']) for x in totalTasks)
            result.setdefault('globalStat', self.getTaskInfo(globalStat, totalLength, completedLength))
            # for i in range(len(activeTasks)):
            #     task = activeTasks[i]
            #     totalTasks[i] = self.getTaskInfo(task)
            for task in totalTasks:
                self.getTaskInfo(task)
            result.setdefault('totalTasks', totalTasks)
        return result

    def getTaskInfo(self, task, totalLength=None, completedLength=None):
        # totalLength = sum(int(x['totalLength']) for x in total)
        # completedLength = sum(int(x['completedLength']) for x in total)
        downloadSpeed = int(task['downloadSpeed'])
        uploadSpeed = int(task['uploadSpeed'])
        totalLength = totalLength if totalLength is not None else int(task['totalLength'])
        completedLength = completedLength if completedLength is not None else int(task['completedLength'])
        remaining = totalLength - completedLength
        task['downloadSpeed'] = format_bytes(downloadSpeed) + '/s'
        task['uploadSpeed'] = format_bytes(uploadSpeed) + '/s'

        if totalLength > 0:
            task['total'] = format(format_bytes(totalLength))
            task['completed'] = format(format_bytes(completedLength))
            task['percent'] = 0.00
            if totalLength:
                task['percent'] = round(completedLength / totalLength, 4)*100
            task['remaining'] = format(format_bytes(remaining))
            if completedLength == totalLength:
                eta = 'finished'
            else:
                eta = 'never'
